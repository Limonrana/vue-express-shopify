...
router.get('/', (req, res) => {
  let shop = req.query.shop;
    ...
    // Redirect
    let state = nonce();
    let redirectUri = conf.redirect_uri + '/api/auth/callback';
      ...
      Shop.findOne({id: shop}, (err, ret) => {
        ...
        let getShop = () => {
          ...
          request.get(shopRequestUrl, {headers: {'X-Shopify-Access-Token': token}}).then((response) => {
            ...
            Shop.updateOne({id: shop}, {$set: {domain: response.domain, scopes: conf.scopes, data: response }}, (err) => {
              if (!err) {
        ...
        if (getShop === null) {
          Shop.create({id: shop, token: token}, getShop)
        } else {
          Shop.updateOne({id: shop}, {$set: {token: token}}, getShop)
        }
      });
      return null;
    }).catch((err) => {
